...
const senMailOrder = (mail,product,total)=>{
    let transporter = nodemailer.createTransport({
        service: "hotmail",
        auth: {
            user: "nguyenducthinh0401@outlook.com", 
...
module.exports = {
    getOrderByUserId:(data)=>{
        return new Promise((resolve,reject)=>{
            Order.find({ userId:data.id}).then(order=>resolve(order)).catch(err=>reject(err))
        })
    },
    createOrder:(data)=>{
        return new Promise((resolve,reject)=>{
            Order.create(data).then(order=>resolve(order)).catch(err=>reject(err))
        })
    },
    getAllOrder:()=>{
        return new Promise((resolve,reject)=>{
            Order.find().then(order=>resolve(order)).catch(err=>reject(err))
        })
    },
    ...
    deleteOrder:(orderId)=>{
        return new Promise((resolve,reject)=>{
            Order.findByIdAndDelete({_id:orderId}).then(order=>resolve(order)).catch(err=>reject(err));
        })
    },
    orderStat:()=>{
        const date = new Date(); 
        ...
        return new Promise(async(resolve,reject)=>{
            try {
                ...
                let sumLastMonth; 
                 if(order.length < 2){
                   sumLastMonth = 0; 
                   sumThisMonth =  order[0].totalMonney;  
                 }else{
        ...
        return new Promise((resolve,reject)=>{
            try {
                const exportExcel = (data, workSheetColumnNames, workSheetName, filePath) => {
                    ...
                    xlsx.writeFile(workBook, path.join('C:\\Users\\nguye\\Documents',filePath));
                }
        
                const exportUsersToExcel = (orders, workSheetColumnNames, workSheetName, filePath) => {
                    const data = orders.map(order => {
                        return [order._id, order.userName, order.email,order.orderDate,order.methodPay,order.status,order.totalOrder];
                    });
