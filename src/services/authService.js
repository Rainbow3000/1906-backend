...
module.exports = {
            ...
            try {
                ...
                if(user){
                    ...
                    if(originPass === data.password){
                        const token = jwt.sign({userName:user.userName,isAdmin:user.isAdmin},process.env.JWT_TOKEN,{
                            expiresIn:"3d"
                        })
    ...
    register:(userInfo)=>{
        return new Promise((resolve,reject)=>{
            ...
            User.create(userInfo).then(user=>resolve(user)).catch(err=>reject(err));
        })
    }
}